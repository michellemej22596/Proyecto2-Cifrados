...
def verify_blockchain_integrity(
    ...
    if is_valid:
        message = (
            f"La blockchain es valida. "
            f"{total_blocks} bloques verificados correctamente."
        )
    else:
        message = (
            "ALERTA: La blockchain ha sido comprometida. "
            "Se detectaron inconsistencias en los hashes."
        )
...
def get_block_by_index(
    ...
    if block is None:
        ...
        raise HTTPException(
            ...
            detail=(
                f"Bloque con indice {block_index} no encontrado. "
                f"La cadena tiene {len(chain)} bloques."
            ),
        )
